...
import { DynamoDB, type QueryOutput } from 'https://aws-api.deno.dev/v0.3/services/dynamodb.ts';
...
export class DynamoDBService {
   ...
   */
  public async queryItemsByIndex(indexName: string, indexValue: string): Promise<QueryOutput.Items> {
   ...
   * @returns QueryOutput.Items
   */
  public async queryItemByPartitionKey(keyName: string, keyValue: string): Promise<QueryOutput.Items> {
    const params = {
      TableName: this.dynamoDBTable,
